...
const navLiks = (col:boolean, clicked:any) => {
  const handleClick=()=>{
    if(clicked)clicked();
}
...
const Header = () => {
  ...
  return (
    <nav className={`flex ${show?"translate-y-0":"-translate-y-28"} ${shadow?"shadow-[0px_10px_30px_-10px_#020c1b]":""} transition-transform duration-500 ease-in-out fixed w-full z-20 bg-bgColor h-28  px-10  justify-between items-center xs-mx:px-4 xs-mx:h-20`}>
